...
export const getCourseProgress = cache(async () => {
    ...
    const firstIncompleteLesson = unitsInActiveCourse
        .flatMap((unit) => unit.lessons) //instead of array of arrays of each unit, get 1 array containing all lessons of all units
        .find((lesson) => {
...
export const getLesson = cache(async (id?: number) => {
    ...
    const normalizedChallenges = data.challenges.map((challenge) => {
        const completed = challenge.challengeProgress && challenge.challengeProgress.length > 0 && challenge.challengeProgress.some((progress) => progress.completed)
